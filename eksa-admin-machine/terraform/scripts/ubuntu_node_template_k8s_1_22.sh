...
read -p 'Final name for EKS-A ubuntu node template, e.g. ubuntu-2004-kube-v1.22: ' ubuntu_template_name
...
export eksd_release_tag=$(file=$(sudo -Hiu ubuntu ls eks-anywhere-downloads/1.22/eks-distro/ | grep kubernetes) | \
...
image-builder build --os ubuntu --hypervisor vsphere --release-channel 1-22 --vsphere-config $HOME/vsphere-connection.json
